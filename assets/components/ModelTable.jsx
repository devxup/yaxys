...
export default class ModelTable extends Component {
  render() {
    ...
    const patchedColumns = columns.map(
      columnOriginal => {
        let column = typeof columnOriginal === "string"
          ? { name: columnOriginal }
          : Object.assign({}, columnOriginal);

        if (typeof columnOriginal === "string" && schema) {
          const property = schema.properties[column.name];
          column.header = (column.name === "id" || column.name === "#")
            ? "#"
        ...
        if (url) {
          column.cell = rowData => {
            return <Link className={ classes.link } to={ url(rowData) }>{ rowData[column.name] }</Link>
          }
        }
        return column;
      }
